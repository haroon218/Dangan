...
@Component({
  selector: 'app-timesheet-detail-rate',
  templateUrl: './timesheet-detail-rate.component.html',
  styleUrls: ['./timesheet-detail-rate.component.css']
})
export class TimesheetDetailRateComponent {
  selectedRow:any={};
  isEdit:boolean;
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<TimesheetDetailRateComponent>,
    private routers: Router,private fb: FormBuilder,
    @Optional() @Inject(MAT_DIALOG_DATA) public data: any
  ) {
    this.shiftIds = this.dataService.employeeInTimeSheets.employeeShifts.map(shift => shift.ShiftId);
  }
  ngOnInit() {
    this.iterateEmpJob();
    ...
    // console.log(this.dataService.employeeInTimeSheet.employeeShifts);
  ...
  removeJob(index: number) {
     this.dataService.employees[0].employeeShifts.splice(index, 1);
  }
  ...
  isShiftSelected(ShiftId: number, currentEmpShift: employeeShift): boolean {
    ...
    return this.dataService.employeeInTimeSheets.employeeShifts.some(empShift => empShift.ShiftId === ShiftId && empShift !== currentEmpShift);
  }
